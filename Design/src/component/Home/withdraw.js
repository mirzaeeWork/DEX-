...
const Withdraw = () => {
    ...
    const [Balloonswithdraw, setBalloonswithdraw] = useState()
    ...
    let UpdateWeb3States = { ...web3States }
     ...
     function withdrawToken() {
        ...
        if (web3States.contractBalloons && web3States.contractDEX) {
            ...
            if (web3States.getLiquidityAcount > amountBalloons
                && amountBalloons > 0) {
                contineuWithdraw()
            } else {
    ...
    async function contineuWithdraw() {

        const DexAddress = await web3States.contractDEX.methods.getContractaddress().call();
        const Amount = Number(amountBalloons) * 10 ** 18;
            web3States.contractDEX.methods.withdraw(Amount.toString()).send({
                from: web3States.account
            }).then(async result => {
                setShow(true)
                setMyShowToast(true)
                setAmountBalloons("")
                let _ethOutput = result.events.LiquidityRemoved.returnValues[2]
    ...
    useEffect( () => {
        setBalloonswithdraw(web3States.getLiquidityAcount)
    },[web3States.getLiquidityAcount]);
            ...
            <Container >
                <Row className='d-flex d-flex justify-content-around mt-3' >
                    <Col  className='d-flex flex-column align-self-stretch py-4 ' style={{ backgroundColor: "rgb(187, 192, 190)" }}>
                        <label className=' mt-1'>مقدار Balloons
                            ( موجودی اکانت برای برداشت توکن از استخر نقدینگی :{Balloonswithdraw ? Balloonswithdraw : null})
                        </label>
                        ...
                        <Button className='mt-3' onClick={withdrawToken} value="success">
                            برداشت از استخر نقدینگی (withdraw)
                        </Button>
                    </Col>
                </Row>
            </Container>
