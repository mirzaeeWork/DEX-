...
const Deposit = () => {
    ...
    const [ethForDeposit, setEthForDeposit] = useState()
    ...
    async function contineuDeposit() {
        ...
        web3States.contractBalloons.methods.approve(DexAddress, Amount.toString()).send({
            ...
            web3States.contractDEX.methods.deposit().send({
                ...
                setMyShowToast(true)
                setAmountETH("")
                let num = web3States.web3.utils.fromWei(result.events.LiquidityProvided.returnValues[1], 'ether');
                setMessage(`مقدار ${Number(num).toFixed(4)}   به استخر نقدینگی اضافه شد`)
                ...
                setShow(true)
                setMessage("انتقال اتر انجام نشد")
    ...
    useEffect( () => {
        setEthForDeposit(web3States.takeOnMsgValueForDeposit)
    },[web3States.takeOnMsgValueForDeposit]);
            ...
            <Container >
                <Row className='d-flex d-flex justify-content-around mt-3' >
                    <Col  className='d-flex flex-column align-self-stretch py-4 ' style={{ backgroundColor: "rgb(187, 192, 190)" }}>
                        <label className=' mt-1'>مقدار ETH
                            (مقدار پیشنهادی اتر برای ارسال کل توکنهای موجود در اکانت:{ethForDeposit ? ethForDeposit : null})                            
                        </label>
                        <label style={{   color: "rgb(160, 15, 4)" }} >چهار رقم اول اعشار وارد شود</label>
                        ...
                        <Button className='mt-3' onClick={deposit} value="success">
                            اضافه کردن به استخر نقدینگی (deposit)
                        </Button>
                    </Col>
                </Row>
            </Container>
